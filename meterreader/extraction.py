...
class DigitalCounterExtraction(VisibleStage):
    def _initWindows(self):
        ...
        cv2.createTrackbar('blur', 'Extraction',
                           self._params['blur'], 15, self.guicb)
        cv2.createTrackbar('thrs1', 'Extraction',
                           self._params['thrs1'], 255, self.guicb)
        cv2.createTrackbar('thrs2', 'Extraction',
                           self._params['thrs2'], 255, self.guicb)
        cv2.createTrackbar('epsilon', 'Extraction',
                           self._params['epsilon'], 255, self.guicb)
    ...
    def _process(self):
        ...
        if contour is not None:
            ...
            extracted = image[int(rect[1]):int(
        ...
        if self._showYourWork:
            ...
            if contour is not None:
                rect = cv2.boundingRect(contour)
                cv2.rectangle(
                    output, (rect[0], rect[1]), (rect[0]+rect[2], rect[1]+rect[3]), (10, 200, 0), 2)
            else:
                cv2.putText(output, "No contour found", (25, 20),
    ...
    def prepare_canny_image(self, image):
        ...
        cannyImage = cv2.Canny(
            blurredImage, self.params['thrs1'], self.params['thrs2'])
        return cannyImage
    ...
    def show_contours(self, contours, output):
        for c in contours:
            ...
            rect = cv2.boundingRect(c)
            cv2.rectangle(output, (rect[0], rect[1]),
